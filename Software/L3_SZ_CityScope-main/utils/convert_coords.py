...
crs_header_4326 = {"type": "name", "properties": {"name": "urn:ogc:def:crs:OGC:1.3:CRS84"}}
crs_header_4547 = {"type": "name", "properties": { "name": "urn:ogc:def:crs:EPSG::4547"}}
...
def safe_save(content, json_path):
    ...
    json.dump(content, open(json_path, 'w', encoding='utf-8'),
...
def convert_pure_coords_from_crs_to_crs(coords, source_crs, save_crs):
    transformer = Transformer.from_crs(source_crs, save_crs)
    new_coords = []
    for coord in coords:
        new_coord = transformer.transform(coord[1], coord[0])
        new_coords.append([new_coord[1], new_coord[0]])
    return new_coords
